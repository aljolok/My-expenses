// =================================================================
// Expense App - Professional JavaScript - Authentication Hotfix
// =================================================================
...
const ui = {
    ...
    render(appState) {
        // This is now the single point of truth for showing/hiding screens
    ...
    // Page creation and other UI functions remain the same as before
    // (createDailyPage, createMonthlyPage, etc.)
    // ... [ The rest of the UI object is identical to the previous version ] ...
    ...
    createDailyPage({ expenses, categories }) {
        ...
        if (dailyExpenses.length > 0) {
            dailyExpenses.forEach(expense => list.appendChild(this.createExpenseItem(expense, categories)));
        } else {
    ...
    createMonthlyPage({ expenses, categories }) {
        ...
        page.innerHTML = `
            <div class="card">
                <h3><i data-lucide="calendar-month"></i>ملخص الشهر</h3>
                <p class="amount">${utils.formatCurrency(total)}</p>
            </div>
            <div class="card">
                <h3><i data-lucide="pie-chart"></i>الأعلى إنفاقًا</h3>
                <div id="monthly-category-list" class="item-list"></div>
            </div>
        `;
        
        const catList = page.querySelector('#monthly-category-list');
                ...
                if(category) {
                    ...
                    catList.innerHTML += `
                        <div class="category-summary-item">
                            ...
                            <span>${utils.formatCurrency(catTotal)}</span>
                        </div>
                    `;
                }
            });
    ...
    createCategoriesPage({ categories }) {
         ...
         page.innerHTML = `
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <input type="color" id="new-category-color" value="#4f46e5">
                </div>
                <button id="add-category-btn" class="btn btn-primary" style="width: 100%; margin-top: var(--space-md);">إضافة فئة</button>
            </div>
        ...
        if (categories.length > 0) {
            ...
            list.innerHTML = `<p class="empty-state">لا توجد فئات. أضف فئة لتبدأ.</p>`;
        }

        return page;
    },
    
    createSettingsPage(appState) {
        const page = document.createElement('div');
        page.className = 'page active';
        page.innerHTML = `
            <div class="card">
                <h3><i data-lucide="settings"></i>الإعدادات</h3>
                <button id="clear-data-btn" class="btn btn-danger" style="width: 100%;">مسح جميع البيانات</button>
            </div>
        `;
        return page;
    },
    ...
    createExpenseItem(expense, categories) {
        ...
        const category = categories.find(c => c.id === expense.categoryId) || { name: 'غير محدد', color: '#9ca3af' };

        item.innerHTML = `
            <div class="item-icon" style="background-color: ${category.color};"><i data-lucide="tag"></i></div>
            <div class="item-details">
                <p class="item-name">${utils.escapeHTML(expense.name)}</p>
                <p class="item-category">${utils.escapeHTML(category.name)}</p>
            </div>
            <p class="item-amount">${utils.formatCurrency(expense.amount)}</p>
            <div class="item-actions">
                <button class="btn-icon edit-expense" data-id="${expense.id}"><i data-lucide="edit-3"></i></button>
                <button class="btn-icon delete-expense" data-id="${expense.id}"><i data-lucide="trash-2"></i></button>
            </div>
        `;
        return item;
    },
    ...
    createCategoryItem(category) {
        ...
        item.innerHTML = `
            <div class="item-icon" style="background-color: ${category.color};"></div>
            <div class="item-details">
                <p class="item-name">${utils.escapeHTML(category.name)}</p>
            </div>
             <div class="item-actions">
                <button class="btn-icon delete-category" data-id="${category.id}"><i data-lucide="trash-2"></i></button>
            </div>
        `;
        return item;
    },
    ...
    setupExpenseModal(expense, categories) {
        ...
        this.elements.expenseModalTitle.textContent = expense ? 'تعديل المصروف' : 'إضافة مصروف';
        this.elements.expenseNameInput.value = expense ? expense.name : '';
        this.elements.expenseAmountInput.value = expense ? expense.amount : '';
        
        if (categories.length === 0) {
            this.elements.expenseCategorySelect.innerHTML = `<option disabled>لا توجد فئات</option>`;
        } else {
             this.elements.expenseCategorySelect.innerHTML = categories
                .map(c => `<option value="${c.id}" ${expense && expense.categoryId === c.id ? 'selected' : ''}>${utils.escapeHTML(c.name)}</option>`)
                .join('');
        }

        this.toggleModal(this.elements.expenseModal, true);
    },
    ...
    showConfirmation(title, message, onConfirm) {
        ...
        const cleanup = () => {
            ...
            this.elements.confirmYesBtn.removeEventListener('click', yesHandler);
            this.elements.confirmNoBtn.removeEventListener('click', noHandler);
        };
        this.elements.confirmYesBtn.addEventListener('click', yesHandler, { once: true });
        this.elements.confirmNoBtn.addEventListener('click', noHandler, { once: true });
    },
    
    handleTheme(isDark) {
        document.documentElement.setAttribute('data-theme', isDark ? 'dark' : 'light');
        this.elements.themeToggleBtn.innerHTML = isDark ? `<i data-lucide="sun"></i>` : `<i data-lucide="moon"></i>`;
        lucide.createIcons();
    }
};
...
const api = {
    ...
    async deleteAllUserData(uid) {
        const expensesQuery = await this.expensesCol(uid).get();
        const categoriesQuery = await this.categoriesCol(uid).get();
        const batch = db.batch();
        expensesQuery.forEach(doc => batch.delete(doc.ref));
        categoriesQuery.forEach(doc => batch.delete(doc.ref));
        return batch.commit();
    },
    listenToExpenses: (uid, callback) => api.expensesCol(uid).onSnapshot(snapshot => callback(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))),
    ...
    async ensureDefaultCategories(uid, currentCategories) {
        if (currentCategories.length > 0) return;
        const defaultCats = [
            { name: 'طعام', color: '#ef4444' }, { name: 'مواصلات', color: '#3b82f6' },
            { name: 'فواتير', color: '#f97316' }, { name: 'تسوق', color: '#10b981' },
        ];
        for (const cat of defaultCats) {
            await this.addCategory(uid, cat);
        }
    }
};


// --- 5. Authentication Logic (HEAVILY REVISED) ---
const authLogic = {
    init() {
        // The core of the fix: onAuthStateChanged is the single source of truth.
        auth.onAuthStateChanged(user => {
            state.cleanupListeners(); // Clear old data listeners
            if (user) {
                // If a user is found (Google or Anonymous), set up data listeners
                ...
                // Update state: user is logged in, no longer loading
                state.setState({ user: user, isLoading: false });
            } else {
                // If no user, simply update state: no user, no longer loading
...
function setupEventListeners() {
    ...
    elements.navButtons.forEach(btn => btn.addEventListener('click', () => {
        ...
        state.setState({ currentPage: btn.dataset.page });
    }));

    elements.themeToggleBtn.addEventListener('click', () => {
        const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
        localStorage.setItem('theme', !isDark ? 'dark' : 'light');
        ui.handleTheme(!isDark);
    });
    ...
    elements.mainApp.addEventListener('click', (e) => {
        const editBtn = e.target.closest('.edit-expense');
        if (editBtn) {
            const expense = state.expenses.find(ex => ex.id === editBtn.dataset.id);
            ui.setupExpenseModal(expense, state.categories);
        }
        
        const deleteBtn = e.target.closest('.delete-expense');
        if (deleteBtn) {
            ui.showConfirmation('حذف المصروف', 'هل أنت متأكد؟', () => {
                api.deleteExpense(state.user.uid, deleteBtn.dataset.id)
                   .then(() => ui.showToast('تم الحذف'))
                   .catch(() => ui.showToast('فشل الحذف', 'error'));
            });
        }

        const addCatBtn = e.target.closest('#add-category-btn');
        if(addCatBtn) handleAddCategory();
        
        const deleteCatBtn = e.target.closest('.delete-category');
        if(deleteCatBtn) {
            ui.showConfirmation('حذف الفئة', 'هل أنت متأكد؟', () => {
                api.deleteCategory(state.user.uid, deleteCatBtn.dataset.id)
                   .then(() => ui.showToast('تم حذف الفئة'))
                   .catch(() => ui.showToast('فشل حذف الفئة', 'error'));
            });
        }
        
        const clearDataBtn = e.target.closest('#clear-data-btn');
        if(clearDataBtn) {
            ui.showConfirmation('مسح جميع البيانات', 'تحذير! سيتم حذف جميع المصروفات والفئات نهائياً.', () => {
                api.deleteAllUserData(state.user.uid)
                   .then(() => ui.showToast('تم مسح جميع البيانات'))
                   .catch(() => ui.showToast('حدث خطأ', 'error'));
            });
        }
    });

    window.addEventListener('online', () => elements.offlineIndicator.classList.add('hidden'));
    window.addEventListener('offline', () => elements.offlineIndicator.classList.remove('hidden'));
}
...
async function handleSaveExpense() {
    ...
    const categoryId = elements.expenseCategorySelect.value;
    ...
    try {
        const action = state.isEditing 
            ? api.updateExpense(state.user.uid, state.isEditing.id, expenseData) 
            : api.addExpense(state.user.uid, expenseData);
        await action;
        ui.showToast(state.isEditing ? "تم تحديث المصروف" : "تمت إضافة المصروف");
        ui.toggleModal(elements.expenseModal, false);
    } catch (error) {
...
async function handleAddCategory() {
    ...
    try {
        await api.addCategory(state.user.uid, { name, color: colorInput.value });
        ui.showToast('تمت إضافة الفئة');
        nameInput.value = '';
    } catch(error) {
        ui.showToast('فشل إضافة الفئة', 'error');
    }
}
...
function init() {
    ...
    setupEventListeners();
    // This is the only place we need to render initially
    // The auth observer will handle all subsequent renders
    ui.render(state);
    authLogic.init();
